...
*   **Positive Prompt Suffix:** Text appended to every prompt the LLM writes, next to the negative override in Image Parameters. Put a character LoRA's trigger words (or a style tag you always want) in there once and stop hoping the model remembers them. It lives in the profile, so it follows the character it belongs to — a LoRA's triggers stay with the LoRA.
...
| :--- | :--- | :--- |
| `"*input*"` | Positive Prompt | The text generated by the LLM based on chat context, with your **Positive Prompt Suffix** appended. |
...
*   **Positive prompt suffix.** Upstream had a negative override and nothing on the positive side, so LoRA trigger words had to be smuggled into the system prompt and hoped for. The suffix is appended to whatever the LLM writes, and lives in the profile alongside the LoRA list.
