...
| :--- | :--- | :--- |
| `"*input*"` | Positive Prompt | The text generated by the LLM based on chat context, with the trigger words of any active LoRA and your **Positive Prompt Suffix** appended. |
...
*   Each row also takes **trigger words**, appended to the prompt while that LoRA is on. Turn the
    LoRA on for the messages that need it and off again afterwards; the words follow the toggle, so
    the prompt never mentions a character LoRA that isn't loaded. They are keyed by filename rather
    than stored in the profile, so a LoRA you use in five profiles is typed once.
...
*   Opening a chat with nothing linked falls back to the profile marked with the crown, so leaving a
    linked character can't leave you generating with its workflow. Staying in that chat keeps
    whatever you picked by hand — a reload of the visible chat (switching connection profiles causes
    one) no longer snaps you back to the crown.
...
*   **Positive prompt suffix.** Upstream had a negative override and nothing on the positive side. The suffix is appended to whatever the LLM writes, and lives in the profile alongside the LoRA list.
*   **Per-LoRA trigger words.** Upstream had nowhere to put them, so triggers went into the prompt by hand and had to be edited back out the moment the LoRA came off. Each LoRA now carries its own, appended only while it is on, keyed by filename so they are typed once and shared across profiles.
*   **Server-routed generation.** Requests to ComfyUI go through the SillyTavern server instead of your browser, so the URL resolves on the SillyTavern machine: no CORS flag, and ComfyUI never has to be reachable from your phone.
*   **LoRA list server plugin.** Optional plugin under `server-plugin/` that fetches the LoRA list server-side, the one list SillyTavern has no proxy route for. Falls back to asking ComfyUI directly when it isn't mounted.
*   **Compression format and quality.** Upstream hardcoded JPEG at 0.9; the toggle now reveals a JPEG/WebP select and a quality slider, and the saved extension is read back from the canvas so a browser that can't encode WebP doesn't leave a mislabeled file.
